...
def _run_inpaint(
    ...
    # Always ensure mask matches image dimensions before inpainting
    img_h, img_w = np_image.shape[:2]
    if np_mask.shape[0] != img_h or np_mask.shape[1] != img_w:
        print(f"[object_remover] Mask/image size mismatch: mask={np_mask.shape[:2]}, image=({img_h},{img_w}). Resizing mask.")
        np_mask = cv2.resize(np_mask, (img_w, img_h), interpolation=cv2.INTER_NEAREST)
...
def _run_sam(image_rgb: np.ndarray, points: list, labels: list) -> str:
    ...
    The mask is ALWAYS resized to match the original image dimensions before encoding.
    """
    h, w = image_rgb.shape[:2]
    print(f"[object_remover] SAM — {len(points)} point(s), image size: {w}x{h}")
    mask = sam_processor.generate_mask(image_rgb, points, labels)
    print(f"[object_remover] SAM raw mask size: {mask.shape[:2]}")

    # Always resize mask to the original image dimensions (SAM internally works at 256px)
    if mask.shape[0] != h or mask.shape[1] != w:
        print(f"[object_remover] Resizing mask from {mask.shape[:2]} → ({h}, {w})")
        mask = cv2.resize(mask, (w, h), interpolation=cv2.INTER_NEAREST)
        mask = cv2.threshold(mask, 127, 255, cv2.THRESH_BINARY)[1]

    print(f"[object_remover] Final mask size: {mask.shape[:2]}")
...
async def inpaint_file(
    ...
    try:
        ...
        # Resize mask to image dimensions if they differ — prevents LaMa internal validation errors
        img_h, img_w = np_image.shape[:2]
        if np_mask.shape[0] != img_h or np_mask.shape[1] != img_w:
            print(f"[inpaint_file] Mask/image mismatch: mask={np_mask.shape[:2]} image=({img_h},{img_w}). Resizing mask to match image.")
            np_mask = cv2.resize(np_mask, (img_w, img_h), interpolation=cv2.INTER_NEAREST)
            np_mask = cv2.threshold(np_mask, 127, 255, cv2.THRESH_BINARY)[1]
